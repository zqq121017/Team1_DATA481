...
log_filename = (
    f"training_logs/{split_folds}_kfold_trial_score_log_"
...
def _write_csv_header(path, hparams):
    ...
    with open(path, mode='w', newline='') as f_out:
        ...
        w.writerow(["# This model predicts the TOTAL SCORE for a trial (sum of all passed rings)"])
        w.writerow([])
...
# ═══════════════════════════════════════════════════════════════════════
# AGGREGATE BY TRIAL: Sum scores, average features across rings
# ═══════════════════════════════════════════════════════════════════════
print("\n" + "="*70)
...
# First, process all the arrays before grouping
def process_branch(df, cols):
    """Convert column values to arrays."""
...
# Add processed arrays back to dataframe for grouping
...
def aggregate_arrays(series):
    """Average arrays across the group."""
    arrays = list(series)
    return np.mean(arrays, axis=0)

trial_aggregated = df_main.groupby(trial_keys).agg({
    'ring_score': 'sum',  # Sum all ring scores in the trial
    'eeg_data': aggregate_arrays,  # Average EEG features
    'act_data': aggregate_arrays,  # Average action features
    'pup_data': aggregate_arrays,  # Average pupil features
    'spc_data': aggregate_arrays,  # Average speech features
}).reset_index()
